...
### Read results and cancel jobs

Use **เปิดอ่านบนเว็บ** to read completed pages in a separate tab, even while the remaining pages are still processing. The reader supports Markdown, tables, local figures and KaTeX equations, with page navigation and a source-Markdown view. Page text requires the job's bearer token; figures use a separate job-scoped HttpOnly cookie. Unfinished pages are unavailable. Only validated local raster-image references can be served; the renderer sanitizes OCR HTML and disables executable/external links. Pinned Marked 18.0.11, DOMPurify 3.4.15 and KaTeX 0.18.7 assets, fonts and licenses are served locally under `app/static/vendor`; VERSION files contain the source npm package integrity hashes.

`POST /api/jobs/{id}/cancel` requires the job bearer token. Queued jobs cancel immediately; running jobs show `cancelling` until the worker stops its inference subprocess. Cancellation remains available when storage is low or the upload rate limit is exhausted. Completed pages and originals are retained, and completed pages remain readable. Uploading the same PDF again can resume the unfinished pages. Cancellation is scoped to the job capability: if another upload is subscribed to the same deduplicated result, its work continues. The shared GPU task stops once every subscriber has cancelled. Cancelled tasks remain cancelled after worker restarts.

Reader endpoints: `GET /api/jobs/{id}/preview/pages/{number}` and the authenticated raster-image URLs returned by that endpoint. `/view#job=...&token=...` is the browser reader; capability tokens remain in URL fragments, not server access logs.
