...
def init():
    ...
    with connect() as c:
        ...
        -- One row per (result, user) already copied to sir-data, so a poll loop pushes once.
...
def claim_push(key, owner_id):
    """True if this caller should push (result, owner) to sir-data. Claimed before the
