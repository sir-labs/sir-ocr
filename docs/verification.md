...
| Check | Evidence |
| --- | --- |
| Automated API/queue/artifact tests | 27 passed, including concurrent deduplication, changed configuration, bad tokens, admission/rate limits, low-space simulation, chunked-size limit, page recovery, OOM retry, validation timeout and job-scoped native-download authorization |
...
## Reader and cancellation update

The isolated browser reader rendered a table, two KaTeX equations and an authenticated figure, navigated to page 2, and removed injected scripts, event handlers and unsafe links. The cancel button moved a queued fixture to cancelled. Automated tests cover cancellation access control/idempotency, low-disk and rate-limit availability, shared subscribers, preserving completed pages, re-upload resumption, cancellation across restart, and termination of a real sleeping inference subprocess (CPU fixture, no production document cancelled). Preview tests reject missing tokens, unfinished pages, traversal paths and SVG/external-image references.

Production follow-up: [deployment 34810856494](https://github.com/sir-labs/sir-ocr/actions/runs/34810856494) passed. The HTTPS reader displayed the existing 2-page acceptance result with a loaded figure and a rendered equation. A new, synthetic 12-page acceptance job was cancelled while recognizing page 2: `cancelling` → `cancelled` in **0.8 seconds**; page 1 remained readable (preview HTTP 200). Worker logs confirm inference subprocess termination and VRAM release; 9,341 MiB GPU memory was free afterward. No user-supplied job was cancelled. Local evidence: `/tmp/sir-ocr-live-cancel-evidence.json`.

## RabbitMQ work queue (2026-09-14, pre-deploy, local)
- RabbitMQ only wakes the worker; SQLite stays the source of truth. The worker acks before GPU work, skips messages whose result is not `queued`, and purges and republishes every queued key from SQLite on each (re)connect. Without a broker it falls back to polling SQLite every `OCR_POLL_SECONDS`.
- Verified against `rabbitmq:4.3.5-alpine`: pytest with a real broker (round trip, queue drains to 0 messages); `docker stop` and `docker kill` mid-consume both raise an AMQP error, and the worker falls back to polling; a refused connection at startup keeps the worker running.
- The service must run as `user: rabbitmq`. A root `rabbitmq-diagnostics` exec before the server is up creates `.erlang.cookie` as root, and the node then crashes with `eacces`.
- Queue arguments in `broker.declare()` are immutable on the live broker. Changing them causes a 406 that is caught as `broker_unavailable`, and the worker stays in poll-only mode. To change them, delete the queue (or the volume) first.
- Compose now requires `OCR_AMQP_USER`/`OCR_AMQP_PASS` (in `~/.config/sir-ocr/deploy.env`). Source that file before running `docker compose logs/ps/down` by hand.
